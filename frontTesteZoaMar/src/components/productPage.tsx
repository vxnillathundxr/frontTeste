...
import { EndH } from './endH';

const ProductPage: React.FC = () => {
    const location = useLocation();
    const { name, price, imagePath, priceWithDiscount, desconto } = location.state || {};
    ...
    if (desconto) {
        risco = (
            <div className="flex flex-col gap-2">
                <div className="flex flex-col gap-3 text-bold md:text-3xl line-through">{price}</div>
                <div className="flex flex-col gap-3 text-bold md:text-7xl ">{priceWithDiscount}</div>
            </div>
        );
    } else {
        risco = (
    ...
    return (
        ...
        <div className="flex md:flex-row flex-col p-10 size h-[98vh] mt-5 gap-32 justify-center bg-zinc-300 rounded">
            <div className="flex flex-col">
                <img className="rounded md:size-[600px] size-72" src={imagePath} alt={name} />
            </div>
            <div className="flex flex-col gap-14">
                <h1 className="font-bold text-7xl">{name}</h1>
                <div>{risco}</div>
                <div className="flex flex-col justify-center items-center gap-16">
                    <p className="font-medium text-2xl">Descrição do produto</p>
                    <div className="flex flex-col gap-5 justify-center items-center">
                        <SizeSelector />
                        <button
