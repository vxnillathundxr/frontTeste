...
const CardProduto: React.FC<CardProdutoProps> = ({name, price, image, newprice, desconto}) =>{
    ...
    if(desconto){
        risco = <div className='flex flex-col gap-2'>
            ...
            <div className='flex flex-col gap-3 text-bold md:text-3xl text-2xl '>R${newprice}</div>
        </div>
    }
    else{
        risco = <div>
                <div className='flex flex-col gap-3 text-bold md:text-2xl text-xl'>R${price}</div>
            </div>
    }
